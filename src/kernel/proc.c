...
#if (CHIP == M68000)
		if (rp == proc_ptr)  printf("last_proc->p_nextready->p_pid = %d\n", last_proc->p_nextready->p_pid);
...
/* takes the first process and puts it at the end of the queue  */
{
  ...
  last_proc->p_nextready = rdy_head[USER_Q];
  rdy_head[USER_Q] = rdy_head[USER_Q]->p_nextready;
  rdy_tail[USER_Q]->p_nextready = NIL_PROC;
  if (next_proc != NIL_PROC){
    last_proc->p_nextready->p_nextready = next_proc;
  }
  return;
}
